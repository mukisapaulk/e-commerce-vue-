  ...
  <div class="container mx-auto p-6">
    ...
    <div v-else class="bg-white p-6 shadow-md rounded-md">
      ...
      <div class="mb-4">
        ...
        <input
          v-model="userStore.userProfile.phone"
          type="text"
          class="border w-full p-2 rounded"
        />
      </div>
      ...
      <div class="mt-6">
        ...
        <ul class="mt-2">
          <li
            v-for="(address, index) in userStore.userProfile.addresses"
            :key="index"
            class="border p-2 rounded mb-2"
          >
            {{ address.street }}, {{ address.city }}, {{ address.state }}, {{ address.zip }},
            {{ address.country }}
            <button @click="userStore.removeAddress(index)" class="text-red-500 ml-2">
              Remove
            </button>
          </li>
        </ul>
      ...
      <div class="mt-6">
        ...
        <ul class="mt-2">
          <li
            v-for="(payment, index) in userStore.userProfile.paymentMethods"
            :key="index"
            class="border p-2 rounded mb-2"
          >
            {{ payment.type }} - **** {{ payment.last4 }} (Exp: {{ payment.expiry }})
            <button @click="userStore.removePaymentMethod(index)" class="text-red-500 ml-2">
              Remove
            </button>
          </li>
        </ul>
        <div class="mt-2 flex gap-2">
          ...
          />
          <input
            v-model="newPaymentMethod.last4"
            type="text"
      ...
      <div class="mt-6">
        ...
        <label class="block mt-2">
          <input
            type="checkbox"
            v-model="userStore.userProfile.preferences.notifications"
            class="mr-2"
          />
        ...
        <label class="block mt-2">
          <select
            v-model="userStore.userProfile.preferences.theme"
            class="border p-2 rounded w-full"
          >
        ...
        <button
          @click="userStore.updatePreferences(userStore.userProfile.preferences)"
...
const addNewPaymentMethod = () => {
  if (
    ...
    !newPaymentMethod.value.last4 ||
    !newPaymentMethod.value.expiry
  )
    return
