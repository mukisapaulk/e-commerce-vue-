    ...
    <div v-else>
      <ul class="space-y-4">
        <li
          ...
          :key="item.id"
          class="flex items-center justify-between p-4 border rounded-lg shadow-md bg-white"
        >
          <!-- Product Image -->
          ...
          <div class="flex items-center">
            <button
              ...
              class="px-2 py-1 bg-gray-300 text-black rounded disabled:opacity-50"
            >
              -
            </button>
            <input
      ...
      <div class="mt-6 p-4 border rounded-lg shadow-md bg-white">
        ...
        <p>
          Tax ({{ (cartStore.taxRate * 100).toFixed(0) }}%): ${{ cartStore.taxAmount.toFixed(2) }}
        </p>
